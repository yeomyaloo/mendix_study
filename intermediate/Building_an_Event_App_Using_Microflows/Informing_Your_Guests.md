...
## 5. 배치 사용
- 이전 단계에서 뉴스레터를 보내기 위한 루프를 만들었으며, 이제 `SUB_Email_CreateAndSend` 마이크로플로우를 설정하겠습니다.
- 이 마이크로플로우는 각 뉴스레터에 대해 호출됩니다.
...
## 6. 이메일 커밋하기
- 방금 만든 이메일을 데이터베이스에 저장하려면 **커밋**해야 합니다.
- 하지만 각 이메일 객체를 루프 내에서 개별적으로 커밋하면 성능 문제가 발생할 수 있으므로, 루프 외부에서 커밋을 진행해야 합니다.
- 여기서는 뉴스레터당 이메일을 한 번에 커밋할 것이며, 배치별로 커밋하는 방식으로 처리합니다.

### 6.1 이메일 커밋 로직 설정
#### 6.1.1 `SUB_Email_CreateAndSend` 마이크로플로우 수정
- **이메일 목록 생성**: 마이크로플로우 시작 부분에 빈 이메일 목록을 만들고, 이를 `EmailListToCommit`이라고 명명합니다.
- **루프 내 이메일 추가**: 각 루프에서 `변경 목록(Change List)` 작업을 사용하여 새로 생성한 이메일 객체를 `EmailListToCommit`에 추가합니다.
  
이 작업을 통해, 루프 내에서 이메일 객체를 개별적으로 커밋하지 않고, 목록에만 추가하게 됩니다.

### 6.2 이메일 목록 커밋
루프가 끝난 후에는, 한 번에 여러 개의 이메일을 커밋하는 방식으로 성능을 최적화할 수 있습니다.
- **커밋 작업 추가**: `EmailListToCommit`에 저장된 이메일 객체들을 한 번에 커밋하기 위해, 루프 외부에 `커밋(Commit)` 작업을 추가합니다.

### 6.3 이메일 목록 초기화
배치가 완료된 후, 다음 배치를 준비하기 위해 `EmailListToCommit`을 초기화해야 합니다.
- **이메일 목록 초기화**: 루프가 끝난 후 `변경 목록(Change List)` 작업을 사용하여, `EmailListToCommit` 목록을 비웁니다. 이 작업은 다음 배치에서 사용할 새로운 빈 목록을 생성하기 위함입니다.

### 6.4 마이크로플로우 구성 예시
마이크로플로우는 다음과 같은 방식으로 구성됩니다:
1. 빈 이메일 목록 `EmailListToCommit` 생성.
2. 각 루프에서 이메일 객체를 `EmailListToCommit`에 추가.
3. 루프가 끝난 후 이메일 목록을 한 번에 커밋.
4. 다음 배치를 위해 이메일 목록 초기화.

이와 같은 방식으로 이메일 커밋을 처리함으로써 성능 문제를 최소화하고, 효율적으로 대량의 이메일을 처리할 수 있습니다.
